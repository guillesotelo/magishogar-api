...
router.get('/getById', async (req, res, next) => {
    try {
        const { _id, isPdf } = req.query

        let resume = {}
        if (isPdf && isPdf !== 'false') resume = await ResumePDF.findOne({ _id }).exec()
        else resume = await Resume.findOne({ _id }).exec()

        if (!resume.email) return res.status(401).json({ message: 'Resume not found' })
